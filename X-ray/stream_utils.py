...
def make_metadata_event(label: str, confidence: float, model: str, label_ar: str = "") -> str:
    """Create SSE event for classification metadata."""
    payload = json.dumps({"label": label, "label_ar": label_ar, "confidence": confidence, "model": model, "type": "metadata"})
